...
\
First, the data is split between a training and test split. The training dataset is used to construct the models, while the testing data set is used to test the accuracy of the models. The training set is thus used to create the recipe for the model whereafter the models are created and the data is fit onto the models.
```{r split-data}
data_split <- data %>% 
...
fit_rf <- rf_final_wf %>% 
  fit(data_train)
```
A table of the models' accuracy is constructed in order to choose which model does the best job in predicting the data.
...
```{r}
data_final_fit1 %>% collect_predictions() %>% custom_metrics(truth = diagnosis, estimate = .pred_class)
```
Due to the high values of the metrics, it is concluded that the model is a good predictor for the real outcome of the diagnosis.
